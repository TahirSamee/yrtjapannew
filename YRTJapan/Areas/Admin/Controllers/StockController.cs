...
namespace YRTJapan.UI.Areas.Admin.Controllers
{
    ...
    public class StockController : Controller
    {
        ...
        [HttpPost]
[ValidateAntiForgeryToken]
public IActionResult Edit(Car car, List<IFormFile> files, List<string> deleteImages)
{
    //if (ModelState.IsValid)
    //{
        _stockRepo.UpdateCarWithImages(car, files, deleteImages);
        return RedirectToAction(nameof(Index));
    //}
            var carli = _stockRepo.GetAllCars().FirstOrDefault(c => c.ID == car.ID);
            // Repopulate images in case of failure
            car.carImages = carli.carImages;
    return View(car);
}
