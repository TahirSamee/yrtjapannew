    ...
    public class StockController : Controller
    {
...

        {
            ...
            var searchModel = new CarSearchViewModel
            {
                ...
                SearchKeyword= !string.IsNullOrEmpty(homeCarFilters.SearchKeyword) ? homeCarFilters.SearchKeyword : string.Empty,
                Offset = 0,
            ...
            int totalLotscars = await _dataService.GetCarDataBySearchCriteriaCountAsync(new CarSearchViewModel { });
            ViewData["TotalCarsInStock"] = "Total Cars In Stock: " + totalLotscars;
            ...
            ViewBag.HomeSearch = homeCarFilters.HomeSearch;
        ...
        public IActionResult AddToFavourites(Favourite favourite)
        {
            ...
            {
                return Json(new { success = true, message = "Login First!" });
            }
        }
        ...
        public async Task<IActionResult> SubmitInquiries([FromBody] SalesInquiryModel inquiryvalues)
        {
            if (ModelState.IsValid)
            {
                ...
                await _context.SaveChangesAsync();
                string emailBody = $"<p>New Sales Inquiries!</p><p><strong>Email:</strong> {salesInquiry.Email}</p><p><strong>Model:</strong> {salesInquiry.Model}</p><p><strong>Message:</strong> {salesInquiry.Message}</p>";
                await _emailSender.SendEmailAsync("info@yrtjapan.com", "New Sales Inquiriesy", emailBody);
        ...
        public async Task<IActionResult> YRTStock()
        {
            var cars = await _dataService.GetAllCars();
            DropDownDataList response = new DropDownDataList();
            response.ListCars = cars;
            response.ListBodyTypes = CommonData.GetAllBodyTypesList;
            response.ListYears = CommonData.GetYearsList;
            response.ListFromPrices = CommonData.GetPriceFromList;
            response.ListToPrices = CommonData.GetPriceToList;
