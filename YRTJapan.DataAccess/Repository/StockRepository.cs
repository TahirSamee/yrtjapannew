...
VALUES (@ID, @LOT, @AUCTION_TYPE, @AUCTION_DATE, @AUCTION, @MARKA_ID, @MODEL_ID, @MARKA_NAME, @MODEL_NAME, @YEAR, @TOWN, @ENG_V, @PW, @KUZOV, @GRADE, @COLOR, @KPP, @KPP_TYPE, @PRIV, @MILEAGE, @EQUIP, @RATE, @START, @FINISH, @STATUS, @TIME, @AVG_PRICE, @AVG_STRING, @IMAGES, @TAG2)";
        ...
        public void UpdateCarWithImages(Car car, List<IFormFile> newImages, List<string> deleteImages)
        {
            using (SqlConnection conn = new SqlConnection(_connectionString))
            {
                conn.Open();

                // ❌ Remove selected images from disk + DB
                if (deleteImages != null && deleteImages.Any())
                {
                    foreach (var imageUrl in deleteImages)
                    {
                        ...
                        var delCmd = new SqlCommand("DELETE FROM CarImages WHERE CarID = @CarID AND ImageUrl = @ImageUrl", conn);
                        delCmd.Parameters.AddWithValue("@CarID", car.ID);
                        delCmd.Parameters.AddWithValue("@ImageUrl", imageUrl);
                ...
                if (newImages != null && newImages.Any())
                {
                    foreach (var file in newImages)
                    {
                        ...
                        var insertImgCmd = new SqlCommand("INSERT INTO CarImages (CarID, ImageUrl) VALUES (@CarID, @ImageUrl)", conn);
                        insertImgCmd.Parameters.AddWithValue("@CarID", car.ID);
                        insertImgCmd.Parameters.AddWithValue("@ImageUrl", imageUrl);
                        insertImgCmd.ExecuteNonQuery();
                    }
                }

                // 🔁 Optionally update the IMAGES field (CSV of all URLs)
                var allImageUrls = new List<string>();

                var getImgsCmd = new SqlCommand("SELECT ImageUrl FROM CarImages WHERE CarID = @CarID", conn);
                getImgsCmd.Parameters.AddWithValue("@CarID", car.ID);
                using (var reader = getImgsCmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        allImageUrls.Add(reader["ImageUrl"].ToString());
                    }
                }
                car.IMAGES = string.Join(",", allImageUrls);

                // 🔄 Update Car record
                UpdateCar(car); // Calls your existing method
            }
        }
